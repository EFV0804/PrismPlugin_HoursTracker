...
import Prism_HeuresAquarium_Variables as prism_var
...
class Prism_HeuresAquarium_Functions(object):
...
# Check if exists/create on disk data files
        self.user_data_dir = 'U:/mesDocuments/HeuresAquarium/'
        ...
        if not os.path.exists(self.user_data_html):
            src = 'R:/Prism/Plugins/beta11.5/HeuresAquarium/templates/hours.html'
            dst = self.user_data_html
            shutil.copy(src, dst)

        if not os.path.exists(self.user_data_html):
            src = 'R:/Prism/Plugins/beta11.5/HeuresAquarium/templates/style.css'
